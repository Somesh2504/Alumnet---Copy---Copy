...
const StudentsManagement = ({ baseURL }) => {
  ...
  const fetchStudents = async () => {
    try {
      setError('');
      const { data } = await axios.get(`${baseURL}api/college/student-records`, {
        withCredentials: true
      });
  ...
  const deleteStudent = async (recordId) => {
    if (window.confirm('Are you sure you want to delete this student record?')) {
      try {
        await axios.delete(`${baseURL}api/college/student-record/${recordId}`, {
          withCredentials: true
        });
        fetchStudents();
      } catch (err) {
...
const AlumniManagement = ({ baseURL }) => {
  ...
  const fetchAlumni = async () => {
    try {
      setError('');
      const { data } = await axios.get(`${baseURL}api/college/alumni-records`, {
        withCredentials: true
      });
  ...
  const deleteAlumni = async (recordId) => {
    if (window.confirm('Are you sure you want to delete this alumni record?')) {
      try {
        await axios.delete(`${baseURL}api/college/alumni-record/${recordId}`, {
          withCredentials: true
        });
        fetchAlumni();
      } catch (err) {
...
const AddRecords = ({ baseURL }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = recordType === 'student' ? `${baseURL}api/college/student-record` : `${baseURL}api/college/alumni-record`;
      const payload = {
        name: formData.name,
